...
WINDOW_TYPE_LIST = [
	...
	'RSS',
	'Calendar',
	'CalendarAttendeesList',
	'CalendarAttendeesTrackingList',
	'Task',
	'Journal',
]
...
class OutlookItemWindow(object):
	...
	def __init__(self, rootDialog, debug=False):
		...
		self.windowType = [wt for wt in WINDOW_TYPE_LIST if getattr(self, 'is' + wt)()]
		log.debug(f"Window types: {self.windowType}")
	...
	def getMessageHeaderFields(self):
		isWriting = len(
			[
				...
				if o.windowControlID == 4256 and controlTypes.State.INVISIBLE not in o.states
			]
		) == 1
		if isWriting:
			dic = {1: (4263, 'From')}
		...
		# hasFromReduced = len([o for o in self.rootDialog.children
		# if o.windowControlID == 4292 and controlTypes.State.INVISIBLE not in o.states]) == 1
		hasFromReduced = len(
			[
				o for o in self.rootDialog.children
				if o.windowControlID == 4280 and controlTypes.State.INVISIBLE not in o.states
			]
		) == 1
		if hasFromReduced:
			dic.update({
	...
	def getMeetingRequestHeaderFields(self):
		...
		hasFromReduced = len(
			[
				o for o in self.rootDialog.children
				if o.windowControlID == 4292 and controlTypes.State.INVISIBLE not in o.states
			]
		) == 1
		if hasFromReduced:
			dic.update({
	...
	def getMeetingReplyHeaderFields(self):
		...
		hasDateTime = len(
			[
				o for o in self.rootDialog.children
				if o.windowControlID == 4105 and controlTypes.State.INVISIBLE not in o.states
			]
		) == 1
		if hasDateTime:
	...
	def getCalendarHeaderFields(self):
		hasOrganizer = len(
			[
				o for o in self.rootDialog.children
				if o.windowControlID == 4523 and controlTypes.State.INVISIBLE not in o.states
			]
		) == 1
		if hasOrganizer:
		...
		dic.update({
			...
			3: (4106, 'To'),
			4: (4100, 'Subject'),
			5: ((4102, 4111), 'Location'),  # Two control ID depending on OL version; maybe 32/64 bits difference
		})
		# Tests if item has detailed Date/Time fields by Checking if start date is visible
		hasDetailedDateTime = len(
			[
				o for o in self.rootDialog.children
				if o.windowControlID == 4098 and controlTypes.State.INVISIBLE not in o.states
			]
		) == 1
		if hasDetailedDateTime:
			...
			hasSingleFieldDateTime = len(
				[
					o for o in self.rootDialog.children
					if o.windowControlID == 4108 and controlTypes.State.INVISIBLE not in o.states
				]
			) == 1
	...
	def getTaskHeaderFields(self):
		...
		hasReminder = len(
			[
				o for o in self.rootDialog.children
				if o.windowControlID == 4226 and controlTypes.State.INVISIBLE not in o.states
			]
		) == 1
		if hasReminder:
			dic.update({
